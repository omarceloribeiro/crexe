...
Data: 2026-09-23. Status: proposta de execução baseada na leitura do repositório.
...
| `crexe doctor` | Diagnosticar caminho/versão da engine instalada, associação efetiva, providers, toolchains e backends de sandbox, sem expor credenciais. |
...
| `crexe test arquivo.crexe` | Executar os testes definidos sobre o projeto preparado, no backend compatível, sem iniciar reparo com LLM silenciosamente. |
...
| Políticas | Receita não amplia permissões, não escolhe endpoint para receber credenciais, não injeta variáveis sensíveis em filhos e não escapa por prefixos ou shell. |
...
7. Projeto gerado, dependências, sandbox, ciclo de correção/testes e pacote exportável.
...
- [ ] Sandbox, build, reparo, testes e empacotamento são gerenciados pelo CREXE; o ciclo completo com Ollama não exige nenhum serviço, SDK de agentes ou sandbox OpenAI.
...
- [ ] Limites de escrita, execução, rede e confiança são aplicados conforme o modo anunciado, sem garantias fictícias.
